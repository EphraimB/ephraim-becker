  ...
  <body>
    <nav>
      <ul>
        ...
        <div id="links">
          <li><a href="../">Home</a></li>
          <li class="focus"><a href="../">Timeline - More info</a></li>
          <div id="dropdown">
    ...
    <main>
      <?php
      $sql = $link->prepare("SELECT *, IFNULL(DATE_FORMAT(concat(EventDate, ' ', EventTime) - INTERVAL EventTimeZoneOffset SECOND, '%Y-%m-%d'), EventDate) AS 'LocalDate', IFNULL(TIME_FORMAT(concat(EventDate, ' ', EventTime) - INTERVAL EventTimeZoneOffset SECOND, '%H:%i:%s'), NULL) AS 'LocalTime' FROM timeline WHERE hide=0 TimelineId=?");
      $sql->bind_param("i", $id);
